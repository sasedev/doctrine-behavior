...
final class ORM extends BaseAdapterORM implements TranslatableAdapter
{
    ...
    public function getDefaultTranslationClass()
    {
        ...
        return 'Sasedev\\Doctrine\\Behavior\\Translatable\\Entity\\Translation';
     ...
     * @return string transformed foreign key
     */
    ...
    public function findTranslation(AbstractWrapper $wrapped, $locale, $field, $translationClass, $objectClass)
    {
        ...
        foreach ($em->getUnitOfWork()
            ->getIdentityMap() as $className => $objects)
        {
            if ($className === $translationClass)
            {
                foreach ($objects as $trans)
                {
                    ...
                    if ($isRequestedTranslation)
                    {
                        ...
                        {
                            $objectId = $this->foreignKey($wrapped->getIdentifier(), $translationClass);
                            $isRequestedTranslation = $trans->getForeignKey() === $objectId && $trans->getObjectClass() === $wrapped->getMetadata()->name;
                        }
                    }
                    if ($isRequestedTranslation)
                    {
        ...
        $qb->select('trans')
            ...
            ->where('trans.locale = :locale', 'trans.field = :field');
    ...
    public function getTranslationValue($object, $field, $value = false)
    {
        ...
        return $type->convertToDatabaseValue($value, $em->getConnection()
            ->getDatabasePlatform());
    ...
    public function setTranslationValue($object, $field, $value)
    {
        ...
        $meta = $wrapped->getMetadata();
        $type = Type::getType($meta->getTypeOfField($field));
        $value = $type->convertToPHPValue($value, $em->getConnection()
            ->getDatabasePlatform());
