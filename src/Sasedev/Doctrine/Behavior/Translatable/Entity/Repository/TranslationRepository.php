...
class TranslationRepository extends EntityRepository
{
    ...
    public function translate($entity, $field, $locale, $value)
    {
        ...
        else
        {
            ...
            $foreignKey = $meta->getReflectionProperty($meta->getSingleIdentifierFieldName())
                ->getValue($entity);
            ...
            if (! $trans)
            {
                ...
                $transMeta->getReflectionProperty('foreignKey')
                    ->setValue($trans, $foreignKey);
                $transMeta->getReflectionProperty('objectClass')
                    ->setValue($trans, $objectClass);
                $transMeta->getReflectionProperty('field')
                    ->setValue($trans, $field);
            ...
            $transformed = $type->convertToDatabaseValue($value, $this->_em->getConnection()
                ->getDatabasePlatform());
            $transMeta->getReflectionProperty('content')
                ->setValue($trans, $transformed);
            if ($needsPersist)
            {
                if ($this->_em->getUnitOfWork()
                    ->isInIdentityMap($entity))
                {
    ...
    public function findTranslations($entity)
    {
        ...
        if ($wrapped->hasValidIdentifier())
        {
            ...
            $config = $this->getTranslatableListener()
