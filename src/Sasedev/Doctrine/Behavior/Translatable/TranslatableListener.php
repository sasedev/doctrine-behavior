...
class TranslatableListener extends MappedEventSubscriber
{
     ...
     */
    const HINT_FALLBACK = 'sasedev.translatable.fallback';

    /**
     * Query hint to override the fallback locale
     */
    const HINT_TRANSLATABLE_LOCALE = 'sasedev.translatable.locale';

    /**
     * Query hint to use inner join strategy for translations
     */
    const HINT_INNER_JOIN = 'sasedev.translatable.inner_join.translations';
     ...
     * in charge.
     * We need to skip issuing additional queries
     * on load
     *
     * @var boolean
     */
    ...
    public function getTranslatableLocale($object, $meta, $om = null)
    {
        ...
        {
            list ($mapping, $parentObject) = $om->getUnitOfWork()
                ->getParentAssociation($object);
    ...
    private function handleTranslatableObjectUpdate(TranslatableAdapter $ea, $object, $isInsert)
    {
        ...
        foreach ($translatableFields as $field)
        {
            ...
            // create new translation if translation not already created and locale is different from default locale, otherwise, we have the date in the
            // original record
     ...
     *
     * @param string $oid
     *            hash of basic entity
     * @param string $field
     *            field of basic entity
     * @param mixed $trans
     *            Translation object
     */
