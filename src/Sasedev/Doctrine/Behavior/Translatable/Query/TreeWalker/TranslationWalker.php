...
class TranslationWalker extends SqlWalker
{
     ...
     * @internal
     */
    const HINT_TRANSLATION_FALLBACKS = '__sasedev.translatable.stored.fallbacks';
     ...
     * @internal
     */
    const HYDRATE_OBJECT_TRANSLATION = '__sasedev.translatable.object.hydrator';
     ...
     * @internal
     */
    const HYDRATE_SIMPLE_OBJECT_TRANSLATION = '__sasedev.translatable.simple_object.hydrator';
    ...
    public function __construct($query, $parserResult, array $queryComponents)
    {
        ...
        $this->platform = $this->getConnection()
            ->getDatabasePlatform();
    ...
    public function walkSelectStatement(SelectStatement $AST)
    {
        ...
        $hydrationMode = $this->getQuery()
            ->getHydrationMode();
        if ($hydrationMode === Query::HYDRATE_OBJECT)
        {
            $this->getQuery()
                ->setHydrationMode(self::HYDRATE_OBJECT_TRANSLATION);
            $this->getEntityManager()
                ->getConfiguration()
                ->addCustomHydrationMode(self::HYDRATE_OBJECT_TRANSLATION, 'Sasedev\\Doctrine\\Behavior\\Translatable\\Hydrator\\ORM\\ObjectHydrator');
            $this->getQuery()
    ...
    private function prepareTranslatedComponents()
    {
        ...
        $quoteStrategy = $em->getConfiguration()
            ->getQuoteStrategy();
        ...
        foreach ($this->translatedComponents as $dqlAlias => $comp)
        {
            ...
            foreach ($config['fields'] as $field)
            {
                ...
                $sql .= ' ON ' . $tblAlias . '.' . $quoteStrategy->getColumnName('locale', $transMeta, $this->platform) . ' = ' . $this->conn->quote($locale);
                $sql .= ' AND ' . $tblAlias . '.' . $quoteStrategy->getColumnName('field', $transMeta, $this->platform) . ' = ' . $this->conn->quote($field);
                ...
                else
                {
                    ...
                    $sql .= ' AND ' . $tblAlias . '.' . $quoteStrategy->getColumnName('foreignKey', $transMeta, $this->platform) . ' = ' . $fkColName;
                }
                ...
                if ((($this->platform instanceof MySqlPlatform) && \in_array($fieldMapping["type"], [
                    "decimal"
                ])) || (! ($this->platform instanceof MySqlPlatform) && ! \in_array($fieldMapping["type"], [
                    "datetime",
                    "datetimetz",
                    "date",
                    "time"
     ...
     *
     * @param $component -
     *            a column with an alias to cast
     * @param $typeFK -
     *            translation table foreign key type
     * @param $typePK -
     *            primary key type which references translation table
