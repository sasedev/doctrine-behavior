...
use Sasedev\Doctrine\Behavior\Exception\UploadableFormSizeException;
use Sasedev\Doctrine\Behavior\Exception\UploadableIniSizeException;
use Sasedev\Doctrine\Behavior\Exception\UploadableNoFileException;
use Sasedev\Doctrine\Behavior\Exception\UploadableNoTmpDirException;
use Sasedev\Doctrine\Behavior\Exception\UploadableUploadException;
use Sasedev\Doctrine\Behavior\Exception\UploadableFileAlreadyExistsException;
use Sasedev\Doctrine\Behavior\Exception\UploadableNoPathDefinedException;
use Sasedev\Doctrine\Behavior\Exception\UploadableMaxSizeException;
use Sasedev\Doctrine\Behavior\Exception\UploadableInvalidMimeTypeException;
use Sasedev\Doctrine\Behavior\Exception\UploadableCouldntGuessMimeTypeException;
...
use Sasedev\Doctrine\Behavior\Uploadable\MimeType\MimeTypeGuesser;
use Sasedev\Doctrine\Behavior\Uploadable\MimeType\MimeTypeGuesserInterface;
use Sasedev\Doctrine\Behavior\Uploadable\Event\UploadablePreFileProcessEventArgs;
use Sasedev\Doctrine\Behavior\Uploadable\Event\UploadablePostFileProcessEventArgs;
...
class UploadableListener extends MappedEventSubscriber
{
     ...
     * @var string
     */
    private $defaultFileInfoClass = 'Sasedev\Doctrine\Behavior\Uploadable\FileInfo\FileInfoArray';
    ...
    /**
     * Array of FileInfoInterface objects.
     * The index is the hash of the entity owner
     ...
     * file field modified.
     * Since we can't mark an entity as "dirty" in the "addEntityFileInfo" method,
    ...
    public function onFlush(EventArgs $args)
    {
        ...
        foreach ($this->fileInfoObjects as $info)
        {
            ...
            $action = ($scheduledForInsert || $scheduledForUpdate) ? ($scheduledForInsert ? self::ACTION_INSERT : self::ACTION_UPDATE) : false;
    ...
    public function processFile(AdapterInterface $ea, $object, $action)
    {
        ...
        if ($evm->hasListeners(Events::UPLOADABLE_PRE_FILE_PROCESS))
        {
            $evm->dispatchEvent(Events::UPLOADABLE_PRE_FILE_PROCESS, new UploadablePreFileProcessEventArgs($this, $om, $config, $fileInfo, $object, $action));
        }
        ...
        $generatorNamespace = 'Sasedev\Doctrine\Behavior\Uploadable\FilenameGenerator';
        ...
        if ($config['callback'] !== '')
        {
            ...
            $callbackMethod->invokeArgs($object, [
                $info
            ]);
        }
        ...
        if ($evm->hasListeners(Events::UPLOADABLE_POST_FILE_PROCESS))
        {
            $evm->dispatchEvent(Events::UPLOADABLE_POST_FILE_PROCESS, new UploadablePostFileProcessEventArgs($this, $om, $config, $fileInfo, $object, $action));
        }
    ...
    protected function getPath(ClassMetadata $meta, array $config, $object)
    {
        ...
        if ($path === '')
        {
            ...
            {
                $pathMethod = $meta->getReflectionClass()
                    ->getMethod($config['pathMethod']);
    ...
    public function moveFile(FileInfoInterface $fileInfo, $path, $filenameGeneratorClass = false, $overwrite = false, $appendNumber = false, $object)
    {
        ...
        {
            $filename = $filenameGeneratorClass::generate(str_replace($path . '/', '', $info['fileWithoutExt']), $info['fileExtension'], $object);
    ...
    /**
     * Simple wrapper method used to move the file.
     ...
     *            Whether this is an uploaded file?
     *
     * @return bool
     */
    ...
    public function setDefaultFileInfoClass($defaultFileInfoClass)
    {
        ...
        $fileInfoInterface = 'Sasedev\\Doctrine\\Behavior\\Uploadable\\FileInfo\\FileInfoInterface';
        $refl = is_string($defaultFileInfoClass) && class_exists($defaultFileInfoClass) ? new \ReflectionClass($defaultFileInfoClass) : false;
