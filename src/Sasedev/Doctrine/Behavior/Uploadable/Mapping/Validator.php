...
class Validator
{
     ...
     * "disallowedTypes" options are BOTH set.
    ...
    public static function validatePath($path)
    {
        ...
        {
            throw new UploadableInvalidPathException('Path must be a string contraining the path to a valid directory.');
        }
    ...
    public static function validateConfiguration(ClassMetadata $meta, array &$config)
    {
        ...
        $config['allowedTypes'] = $config['allowedTypes'] ? (strpos($config['allowedTypes'], ',') !== false ? explode(',', $config['allowedTypes']) : [
            $config['allowedTypes']
        ]) : false;
        $config['disallowedTypes'] = $config['disallowedTypes'] ? (strpos($config['disallowedTypes'], ',') !== false ? explode(',', $config['disallowedTypes']) : [
            $config['disallowedTypes']
        ]) : false;
        ...
        switch ((string) $config['filenameGenerator'])
        {
            ...
            default:
                ...
                if (class_exists($config['filenameGenerator']))
                {
                    ...
                    if ($refl->implementsInterface('Sasedev\Doctrine\Behavior\Uploadable\FilenameGenerator\FilenameGeneratorInterface'))
                    {
                        $ok = true;
                    }
                }
