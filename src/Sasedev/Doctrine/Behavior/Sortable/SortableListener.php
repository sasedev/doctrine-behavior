...
class SortableListener extends MappedEventSubscriber
{
    ...
    protected function processInsert(SortableAdapter $ea, array $config, $meta, $object)
    {
        ...
        $old = $meta->getReflectionProperty($config['position'])
            ->getValue($object);
        $newPosition = $meta->getReflectionProperty($config['position'])
            ->getValue($object);
    ...
    protected function processUpdate(SortableAdapter $ea, array $config, $meta, $object)
    {
        ...
        if ($changed)
        {
            ...
            {
                $oldPosition = $meta->getReflectionProperty($config['position'])
                    ->getValue($object);
            }
    ...
    protected function processDeletion(SortableAdapter $ea, array $config, $meta, $object)
    {
        ...
        $position = $meta->getReflectionProperty($config['position'])
            ->getValue($object);
            ...
            foreach ($relocation['deltas'] as $delta)
            {
                ...
                foreach ($uow->getIdentityMap() as $className => $objects)
                {
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $pos = $meta->getReflectionProperty($config['position'])
                            ->getValue($object);
                        ...
                        {
                            $gr = $meta->getReflectionProperty($group)
                                ->getValue($object);
                            if (null === $value)
                            {
                                $matches = $gr === null;
                        ...
                        if ($matches)
                        {
                            // We cannot use `$this->setFieldValue()` here, because it will create a change set, that
                            // will
                            ...
                            $meta->getReflectionProperty($config['position'])
                                ->setValue($object, $updatedObjects[$oid]['newValue']);
                        }
                    }
                }
            }
    ...
    protected function getMaxPosition(SortableAdapter $ea, $meta, $config, $object, array $groups = [])
    {
        ...
        {
            if (is_object($val) && ($uow->isScheduledForInsert($val) || ! $em->getMetadataFactory()
                ->isTransient(ClassUtils::getClass($val)) && $uow::STATE_MANAGED !== $ea->getObjectState($uow, $val)))
            {
                return - 1;
            }
        }
     ...
     *
     * @param string $hash
     *            The hash of the sorting group
     * @param string $class
     *            The object class
     * @param array $groups
     *            The sorting groups
     * @param int $start
     *            Inclusive index to start relocation from
     * @param int $stop
     *            Exclusive index to stop relocation at
     * @param int $delta
     *            The delta to add to relocated nodes
     * @param array $exclude
     *            Objects to be excluded from relocation
     */
    protected function addRelocation($hash, $class, $groups, $start, $stop, $delta, array $exclude = [])
    {
        ...
        try
        {
            ...
            array_walk($this->relocations[$hash]['deltas'],
                function (&$val, $idx, $needle)
                {
    ...
    protected function getGroups($meta, $config, $object)
    {
        ...
        if (isset($config['groups']))
        {
            ...
            {
                $groups[$group] = $meta->getReflectionProperty($group)
                    ->getValue($object);
            }
        }
