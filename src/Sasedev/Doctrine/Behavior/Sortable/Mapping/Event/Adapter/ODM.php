<?php
namespace Sasedev\Doctrine\Behavior\Sortable\Mapping\Event\Adapter;
...
final class ODM extends BaseAdapterODM implements SortableAdapter
{
    ...
    public function getMaxPosition(array $config, $meta, $groups)
    {
        ...
        {
            if (is_object($value) && ! $dm->getMetadataFactory()
                ->isTransient(ClassUtils::getClass($value)))
            {
                $qb->field($group)
                    ->references($value);
            }
        ...
        $document = $qb->getQuery()
            ->getSingleResult();

        if ($document)
        {
            return $meta->getReflectionProperty($config['position'])
                ->getValue($document);
        }
