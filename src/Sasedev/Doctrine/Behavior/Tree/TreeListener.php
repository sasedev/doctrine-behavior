...
class TreeListener extends MappedEventSubscriber
{
    ...
    public function onFlush(EventArgs $args)
    {
        ...
        foreach ($ea->getScheduledObjectInsertions($uow) as $object)
        {
            ...
            if ($this->getConfiguration($om, $meta->name))
            {
                ...
                $this->getStrategy($om, $meta->name)
                    ->processScheduledInsertion($om, $object, $ea);
        ...
        foreach ($ea->getScheduledObjectUpdates($uow) as $object)
        {
            ...
            if ($this->getConfiguration($om, $meta->name))
            {
                $this->usedClassesOnFlush[$meta->name] = null;
                $this->getStrategy($om, $meta->name)
                    ->processScheduledUpdate($om, $object, $ea);
            }
        }
        ...
        foreach ($ea->getScheduledObjectDeletions($uow) as $object)
        {
            ...
            if ($this->getConfiguration($om, $meta->name))
            {
                $this->usedClassesOnFlush[$meta->name] = null;
                $this->getStrategy($om, $meta->name)
                    ->processScheduledDelete($om, $object);
            }
        }
    ...
    public function postPersist(EventArgs $args)
    {
        ...
        {
            $this->getStrategy($om, $meta->name)
                ->processPostPersist($om, $object, $ea);
        }
    ...
    public function postUpdate(EventArgs $args)
    {
        ...
        {
            $this->getStrategy($om, $meta->name)
                ->processPostUpdate($om, $object, $ea);
        }
    ...
    public function postRemove(EventArgs $args)
    {
        ...
        {
            $this->getStrategy($om, $meta->name)
                ->processPostRemove($om, $object, $ea);
        }
