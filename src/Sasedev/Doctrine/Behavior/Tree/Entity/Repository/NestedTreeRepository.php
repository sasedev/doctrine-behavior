 ...
 * to interact with NestedSet tree.
 * Repository uses
...
class NestedTreeRepository extends AbstractTreeRepository
{
    ...
    public function getRootNodes($sortByField = null, $direction = 'asc')
    {
        ...
        return $this->getRootNodesQuery($sortByField, $direction)
            ->getResult();
    ...
    public function __call($method, $args)
    {
        ...
        if (substr($method, 0, 9) === 'persistAs')
        {
            ...
            $this->listener->getStrategy($this->_em, $meta->name)
    ...
    public function getPathQueryBuilder($node)
    {
        ...
        $qb->select('node')
            ->from($config['useObjectClass'], 'node')
            ->where($qb->expr()
            ->lte('node.' . $config['left'], $left))
            ->andWhere($qb->expr()
            ->gte('node.' . $config['right'], $right))
            ->orderBy('node.' . $config['left'], 'ASC');
        if (isset($config['root']))
        {
            $rootId = $wrapped->getPropertyValue($config['root']);
            $qb->andWhere($qb->expr()
                ->eq('node.' . $config['root'], ':rid'));
        ...
        if ($node !== null)
        {
            if ($node instanceof $meta->name)
            {
                ...
                else
                {
                    ...
                    {
                        $qb->where($qb->expr()
                            ->lt('node.' . $config['right'], $right));
                        $qb->andWhere($qb->expr()
                            ->gt('node.' . $config['left'], $left));
                    }
                }
                if (isset($config['root']))
                {
     ...
     *
     * @param object $root
     *            - root node in case of root tree is required
    ...
    public function getLeafsQueryBuilder($root = null, $sortByField = null, $direction = 'ASC')
    {
        ...
        $qb->select('node')
            ->from($config['useObjectClass'], 'node')
            ->where($qb->expr()
            ->eq('node.' . $config['right'], '1 + node.' . $config['left']));
        if (isset($config['root']))
        {
    ...
    public function getLeafsQuery($root = null, $sortByField = null, $direction = 'ASC')
    {
        ...
        return $this->getLeafsQueryBuilder($root, $sortByField, $direction)
            ->getQuery();
    ...
    public function getLeafs($root = null, $sortByField = null, $direction = 'ASC')
    {
        ...
        return $this->getLeafsQuery($root, $sortByField, $direction)
            ->getResult();
    ...
    public function getNextSiblingsQueryBuilder($node, $includeSelf = false)
    {
        ...
        $qb->select('node')
            ->from($config['useObjectClass'], 'node')
            ->where($includeSelf ? $qb->expr()
            ->gte('node.' . $config['left'], $left) : $qb->expr()
            ->gt('node.' . $config['left'], $left))
            ->orderBy("node.{$config['left']}", 'ASC');
        if ($parent)
        {
    ...
    public function getNextSiblingsQuery($node, $includeSelf = false)
    {
        ...
        return $this->getNextSiblingsQueryBuilder($node, $includeSelf)
            ->getQuery();
    ...
    public function getNextSiblings($node, $includeSelf = false)
    {
        ...
        return $this->getNextSiblingsQuery($node, $includeSelf)
            ->getResult();
    ...
    public function getPrevSiblingsQueryBuilder($node, $includeSelf = false)
    {
        ...
        $qb->select('node')
            ->from($config['useObjectClass'], 'node')
            ->where($includeSelf ? $qb->expr()
            ->lte('node.' . $config['left'], $left) : $qb->expr()
            ->lt('node.' . $config['left'], $left))
            ->orderBy("node.{$config['left']}", 'ASC');
        if ($parent)
        {
            $wrappedParent = new EntityWrapper($parent, $this->_em);
    ...
    public function getPrevSiblingsQuery($node, $includeSelf = false)
    {
        ...
        return $this->getPrevSiblingsQueryBuilder($node, $includeSelf)
            ->getQuery();
    ...
    public function getPrevSiblings($node, $includeSelf = false)
    {
        ...
        return $this->getPrevSiblingsQuery($node, $includeSelf)
            ->getResult();
            ...
            try
            {
                ...
                if (isset($config['root']) && ! $parent)
                {
                    ...
                    foreach ($nodes as $newRoot)
                    {
                        ...
                        $qb->setParameter('rid', $rootId);
                        $qb->where($qb->expr()
                            ->eq('node.' . $config['root'], ':rpid'));
                        $qb->setParameter('rpid', $nodeId);
                        $qb->andWhere($qb->expr()
                            ->gte('node.' . $config['left'], $left));
                        $qb->andWhere($qb->expr()
                            ->lte('node.' . $config['right'], $right));
                        $qb->getQuery()
     ...
     *            - node from which to start reordering the tree; null will reorder everything
     * @param string $sortByField
     *            - field name to sort by
     * @param string $direction
     *            - sort direction : "ASC" or "DESC"
     * @param boolean $verify
     *            - true to verify tree first
     *
     * @return bool|null
     */
    ...
    public function recover()
    {
        ...
        $doRecover = function ($root, &$count, &$lvl) use ($meta, $config, $self, $em, &$doRecover)
        {
            ...
            $rgt = $count ++;
            $meta->getReflectionProperty($config['left'])
                ->setValue($root, $lft);
            $meta->getReflectionProperty($config['right'])
                ->setValue($root, $rgt);
            if (isset($config['level']))
            {
                $meta->getReflectionProperty($config['level'])
                    ->setValue($root, $lvl);
            }
            $em->persist($root);
        };
    ...
    public function getNodesHierarchyQueryBuilder($node = null, $direct = false, array $options = [], $includeNode = false)
    {
        ...
        return $this->childrenQueryBuilder($node, $direct, isset($config['root']) ? [
            $config['root'],
            $config['left']
        ] : $config['left'], 'ASC', $includeNode);
    ...
    public function getNodesHierarchy($node = null, $direct = false, array $options = [], $includeNode = false)
    {
        ...
        return $this->getNodesHierarchyQuery($node, $direct, $options, $includeNode)
            ->getArrayResult();
    ...
    protected function validate()
    {
        ...
        return $this->listener->getStrategy($this->_em, $this->getClassMetadata()->name)
            ->getName() === Strategy::NESTED;
    ...
    private function verifyTree(&$errors, $root = null)
    {
        ...
        if (isset($config['root']))
        {
            if (isset($config['root']))
            {
                $rootId = $meta->getReflectionProperty($config['root'])
                    ->getValue($root);
                if (is_object($rootId))
                {
                    $rootId = $meta->getReflectionProperty($identifier)
                        ->getValue($rootId);
                }
            }
            else
        ...
        $min = intval($qb->getQuery()
            ->getSingleScalarResult());
        $edge = $this->listener->getStrategy($this->_em, $meta->name)
            ->max($this->_em, $config['useObjectClass'], $rootId);
        // check duplicate right and left values
        for ($i = $min; $i <= $edge; $i ++)
        {
            $qb = $this->getQueryBuilder();
            $qb->select($qb->expr()
                ->count('node.' . $identifier))
                ->from($config['useObjectClass'], 'node')
                ->where($qb->expr()
                ->orX($qb->expr()
                ->eq('node.' . $config['left'], $i), $qb->expr()
                ->eq('node.' . $config['right'], $i)));
            if (isset($config['root']))
            {
                $qb->andWhere($qb->expr()
                    ->eq('node.' . $config['root'], ':rid'));
        ...
        $qb->select('node')
            ...
            ->where($qb->expr()
            ->isNotNull('node.' . $config['parent']))
            ->andWhere($qb->expr()
            ->isNull('parent.' . $identifier));
        if (isset($config['root']))
        {
            $qb->andWhere($qb->expr()
                ->eq('node.' . $config['root'], ':rid'));
        ...
        $qb->select('node')
            ->from($config['useObjectClass'], 'node')
            ->where($qb->expr()
            ->lt('node.' . $config['right'], 'node.' . $config['left']));
        if (isset($config['root']))
        {
            $qb->andWhere($qb->expr()
                ->eq('node.' . $config['root'], ':rid'));
        ...
        foreach ($nodes as $node)
        {
            $right = $meta->getReflectionProperty($config['right'])
                ->getValue($node);
            $left = $meta->getReflectionProperty($config['left'])
                ->getValue($node);
            $id = $meta->getReflectionProperty($identifier)
                ->getValue($node);
            ...
            elseif ($parent)
            {
                ...
                $parentRight = $meta->getReflectionProperty($config['right'])
                    ->getValue($parent);
                $parentLeft = $meta->getReflectionProperty($config['left'])
                    ->getValue($parent);
                $parentId = $meta->getReflectionProperty($identifier)
                    ->getValue($parent);
                if ($left < $parentLeft)
                {
    ...
    private function removeSingle(EntityWrapper $wrapped)
    {
        ...
        $this->_em->getUnitOfWork()
            ->removeFromIdentityMap($wrapped->getObject());
