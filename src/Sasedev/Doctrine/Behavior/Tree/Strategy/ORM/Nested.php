...
class Nested implements Strategy
{
    ...
    public function processScheduledInsertion($em, $node, AdapterInterface $ea)
    {
        ...
        {
            $meta->getReflectionProperty($config['root'])
                ->setValue($node, 0);
        }
        else
            if (isset($config['rootIdentifierMethod']) && is_null($meta->getReflectionProperty($config['root'])
                ->getValue($node)))
            {
                $meta->getReflectionProperty($config['root'])
                    ->setValue($node, 0);
    ...
    public function updateNode(EntityManagerInterface $em, $node, $parent, $position = 'FirstChild')
    {
            ...
            switch ($position)
            {
                case self::PREV_SIBLING:
                    ...
                    else
                    {
                        ...
                        $em->getUnitOfWork()
                            ->recomputeSingleEntityChangeSet($meta, $node);
                        $start = $parentLeft;
                    }
                    break;
        ...
        elseif (! isset($config['root']) || ($meta->isSingleValuedAssociation($config['root']) && ($newRoot = $meta->getFieldValue($node, $config['root']))))
        {
        ...
        else
        {
            ...
            if (isset($config['rootIdentifierMethod']))
            {
                ...
                $criteria->andWhere(Criteria::expr()->notIn($wrapped->getMetadata()->identifier[0], [
                ...
                $criteria->orderBy([
                    $config['right'] => Criteria::ASC
                ]);
                $roots = $repo->matching($criteria)
                    ->toArray();
        ...
        if (! $isNewNode)
        {
            ...
            $this->shiftRangeRL($em, $config['useObjectClass'], $left, $right, $diff, $root, $newRoot, $levelDiff);
        ...
        else
        {
            ...
            if (isset($config['root']))
            {
                ...
                $em->getUnitOfWork()
                    ->setOriginalEntityProperty($oid, $config['root'], $newRoot);
            }
            if (isset($config['level']))
            {
                ...
                $em->getUnitOfWork()
                    ->setOriginalEntityProperty($oid, $config['level'], $level);
            }
            if (isset($newParent))
            {
            ...
            $em->getUnitOfWork()
                ->setOriginalEntityProperty($oid, $config['left'], $left + $diff);
            $em->getUnitOfWork()
                ->setOriginalEntityProperty($oid, $config['right'], $right + $diff);
        }
    ...
    public function shiftRL(EntityManagerInterface $em, $class, $first, $delta, $root = null)
    {
        ...
        foreach ($em->getUnitOfWork()
            ->getIdentityMap() as $className => $nodes)
        {
            ...
            foreach ($nodes as $node)
            {
                ...
                $left = $meta->getReflectionProperty($config['left'])
                    ->getValue($node);
                $currentRoot = isset($config['root']) ? $meta->getReflectionProperty($config['root'])
                    ->getValue($node) : null;
                if ($currentRoot === $root && $left >= $first)
                {
                    $meta->getReflectionProperty($config['left'])
                        ->setValue($node, $left + $delta);
                    $em->getUnitOfWork()
                        ->setOriginalEntityProperty($oid, $config['left'], $left + $delta);
                }
                ...
                {
                    $meta->getReflectionProperty($config['right'])
                        ->setValue($node, $right + $delta);
                    $em->getUnitOfWork()
                        ->setOriginalEntityProperty($oid, $config['right'], $right + $delta);
                }
            }
        }
            ...
            ->getIdentityMap() as $className => $nodes)
        {
            ...
            foreach ($nodes as $node)
            {
                ...
                if ($currentRoot === $root && $left >= $first && $right <= $last)
                {
                    ...
                    {
                        $meta->getReflectionProperty($config['root'])
                            ->setValue($node, $destRoot);
                        $uow->setOriginalEntityProperty($oid, $config['root'], $destRoot);
                    }
                    if (isset($config['level']))
                    {
                        $level = $meta->getReflectionProperty($config['level'])
                            ->getValue($node);
                        $meta->getReflectionProperty($config['level'])
                            ->setValue($node, $level + $levelDelta);
