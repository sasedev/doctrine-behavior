...
class Closure implements Strategy
{
     ...
     * new nodes.
     * They have to wait until their parents are inserted
    ...
    public function processMetadataLoad($em, $meta)
    {
        ...
        $closureMetadata->table['uniqueConstraints'][$indexName] = [
            'columns' => [
                $this->getJoinColumnFieldName($em->getClassMetadata($config['closure'])
                    ->getAssociationMapping('ancestor')),
                $this->getJoinColumnFieldName($em->getClassMetadata($config['closure'])
                    ->getAssociationMapping('descendant'))
            ]
        ];
    ...
    public function processPostPersist($em, $entity, AdapterInterface $ea)
    {
        ...
        while ($node = array_shift($this->pendingChildNodeInserts[$emHash]))
        {
            ...
            $nodeId = $meta->getReflectionProperty($identifier)
                ->getValue($node);
            ...
            $ancestorColumnName = $this->getJoinColumnFieldName($em->getClassMetadata($config['closure'])
                ->getAssociationMapping('ancestor'));
            $descendantColumnName = $this->getJoinColumnFieldName($em->getClassMetadata($config['closure'])
                ->getAssociationMapping('descendant'));
            $depthColumnName = $em->getClassMetadata($config['closure'])
                ->getColumnName('depth');
    ...
    protected function setLevelFieldOnPendingNodes(ObjectManager $em)
    {
        ...
        if (! empty($this->pendingNodesLevelProcess))
        {
            ...
            {
                $children = $em->getRepository($meta->name)
                    ->children($node);
            ...
            $levelsAssoc = $em->getConnection()
                ->executeQuery($sql, [
                \array_keys($this->pendingNodesLevelProcess)
            ], [
                $type
            ])
    ...
    public function updateNode(EntityManagerInterface $em, $node, $oldParent)
    {
        ...
        if ($oldParent)
        {
            ...
            $ids = $conn->executeQuery($subQuery, compact('nodeId'))
