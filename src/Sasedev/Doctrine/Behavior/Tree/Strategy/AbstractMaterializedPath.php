<?php
namespace Sasedev\Doctrine\Behavior\Tree\Strategy;
...
abstract class AbstractMaterializedPath implements Strategy
{
    ...
    public function processPreLockingActions($om, $node, $action)
    {
        ...
        if ($config['activate_locking'])
        {
            ...
            if ($parentNode !== $node && $parentNode instanceof \ProxyManager\Proxy\GhostObjectInterface)
            {
            ...
            if (! is_null($lockTime))
            {
                $lockTime = $lockTime instanceof \MongoDB\BSON\UTCDatetime ? $lockTime->sec : $lockTime->getTimestamp();
            }
