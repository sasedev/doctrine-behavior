    ...
    public function readExtendedMetadata($meta, array &$config)
    {
        ...
        if (isset($mapping['id']))
        {
            foreach ($mapping['id'] as $field => $fieldMapping)
            {
                ...
                {
                    if (\in_array('treePathSource', $fieldMapping['sasedev']))
                    {
        ...
        if (isset($mapping['fields']))
        {
            foreach ($mapping['fields'] as $field => $fieldMapping)
            {
                if (isset($fieldMapping['sasedev']))
                {
                    if (\in_array('treeLeft', $fieldMapping['sasedev']))
                    {
                    ...
                    elseif (\in_array('treeRight', $fieldMapping['sasedev']))
                    {
                        if (! $validator->isValidField($meta, $field))
                        {
                    ...
                    elseif (\in_array('treeLevel', $fieldMapping['sasedev']))
                    {
                        if (! $validator->isValidField($meta, $field))
                        {
                    ...
                    elseif (\in_array('treeRoot', $fieldMapping['sasedev']))
                    {
                    ...
                    elseif (\in_array('treePath', $fieldMapping['sasedev']) || isset($fieldMapping['sasedev']['treePath']))
                    {
                    ...
                    elseif (\in_array('treePathSource', $fieldMapping['sasedev']))
                    {
                        if (! $validator->isValidFieldForPathSource($meta, $field))
                        {
                            throw new InvalidMappingException(
                    ...
                    elseif (\in_array('treePathHash', $fieldMapping['sasedev']))
                    {
                        if (! $validator->isValidFieldForPathSource($meta, $field))
                        {
                            throw new InvalidMappingException(
                    ...
                    elseif (\in_array('treeLockTime', $fieldMapping['sasedev']))
                    {
                    ...
                    elseif (\in_array('treeParent', $fieldMapping['sasedev']))
                    {
                        $config['parent'] = $field;
                    }
                }
        ...
        if (isset($mapping['manyToOne']))
        {
            ...
            {
                if (isset($relationMapping['sasedev']))
                {
                    if (\in_array('treeParent', $relationMapping['sasedev']))
                    {
                    ...
                    if (\in_array('treeRoot', $relationMapping['sasedev']))
                    {
                        // if (! $rel = $this->getRelatedClassName($meta, $relationMapping['targetEntity'])) {
                        if (! $this->getRelatedClassName($meta, $relationMapping['targetEntity']))
                        {
