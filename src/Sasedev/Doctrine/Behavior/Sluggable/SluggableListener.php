<?php
namespace Sasedev\Doctrine\Behavior\Sluggable;
...
class SluggableListener extends MappedEventSubscriber
{
    ...
    private $transliterator = [
        'Sasedev\Doctrine\Behavior\Sluggable\Util\Urlizer',
        'transliterate'
    ];
    ...
    public function prePersist(EventArgs $args)
    {
        ...
        if ($config = $this->getConfiguration($om, $meta->name))
        {
            ...
            foreach (\array_keys($config['slugs']) as $slugField)
            {
                ...
                {
                    $meta->getReflectionProperty($slugField)
                        ->setValue($object, '__id__');
                }
            }
        }
    ...
    private function generateSlug(SluggableAdapter $ea, $object)
    {
        ...
        foreach ($config['slugs'] as $slugField => $options)
        {
            ...
            $slug = $meta->getReflectionProperty($slugField)
                ->getValue($object);
            ...
            if (null === $slug || $slug === '__id__' || ! isset($changeSet[$slugField]))
            {
                ...
                foreach ($options['fields'] as $sluggableField)
                {
                    ...
                    $value = $meta->getReflectionProperty($sluggableField)
                        ->getValue($object);
    ...
    private function makeUniqueSlug(SluggableAdapter $ea, $object, $preferredSlug, $recursing = false, $config = [])
    {
        ...
        {
            $base = $meta->getReflectionProperty($config['unique_base'])
                ->getValue($object);
        }
        ...
        if (isset($this->persisted[$class = $ea->getRootObjectClass($meta)]))
        {
            ...
            {
                if ($base !== false && $meta->getReflectionProperty($config['unique_base'])
                    ->getValue($obj) !== $base)
                {
                    continue; // if unique_base field is not the same, do not take slug as similar
                }
                $slug = $meta->getReflectionProperty($config['slug'])
                    ->getValue($obj);
