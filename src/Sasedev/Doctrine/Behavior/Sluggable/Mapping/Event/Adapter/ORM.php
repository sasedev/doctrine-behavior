<?php
namespace Sasedev\Doctrine\Behavior\Sluggable\Mapping\Event\Adapter;
...
class ORM extends BaseAdapterORM implements SluggableAdapter
{
    ...
    public function getSimilarSlugs($object, $meta, array $config, $slug)
    {
        ...
        if ($config['unique'] && isset($config['unique_base']))
        {
            ...
            if (array_key_exists($config['unique_base'], $wrapped->getMetadata()
                ->getAssociationMappings()))
            {
                $mapping = $wrapped->getMetadata()
                    ->getAssociationMapping($config['unique_base']);
            }
            else
            {
                $mapping = false;
            ...
            elseif ($ubase && $mapping && \in_array($mapping['type'], [
            ...
            ]))
            {
                ...
                foreach (array_keys($mapping['targetToSourceKeyColumns']) as $i => $mappedKey)
                {
                    ...
                    $qb->andWhere($qb->expr()
                        ->eq($mappedAlias . '.' . $mappedProp, ':assoc' . $i));
