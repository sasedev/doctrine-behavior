...
use Sasedev\Doctrine\Behavior\Sluggable\SluggableListener;
 ...
 * a slug of related object.
 * For instance user may belong to a company
...
class RelativeSlugHandler implements SlugHandlerInterface
{
    ...
    public function onChangeDecision(SluggableAdapter $ea, array &$config, $object, &$slug, &$needToChangeSlug)
    {
        ...
        $isInsert = $this->om->getUnitOfWork()
    ...
    public function postSlugBuild(SluggableAdapter $ea, array &$config, $object, &$slug)
    {
        ...
        $this->sluggable->setTransliterator([
            $this,
            'transliterate'
        ]);
    ...
    public function transliterate($text, $separator, $object)
    {
        ...
        $result = call_user_func_array($this->originalTransliterator, [
            $text,
            $separator,
            $object
        ]);
        ...
        if ($relation)
        {
            ...
            {
                $slug = call_user_func_array($this->originalTransliterator, [
                    $slug,
                    $separator,
                    $object
                ]);
