 ...
 * used together with RelativeSlugHandler.
 * Updates back related slug on
 * relation changes
 *
            ...
            if ($mappedByConfig)
            {
                ...
                foreach ($uow->getIdentityMap() as $className => $objects)
                {
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $objectSlug = $meta->getReflectionProperty($mappedByConfig['slug'])
                            ->getValue($object);
                        if (preg_match("@^{$oldSlug}@smi", $objectSlug))
                        {
                            $objectSlug = str_replace($oldSlug, $slug, $objectSlug);
                            $meta->getReflectionProperty($mappedByConfig['slug'])
                                ->setValue($object, $objectSlug);
