...
abstract class AbstractTrackingListener extends MappedEventSubscriber
{
        ...
        foreach ($all as $object)
        {
            ...
            if ($uow->isScheduledForInsert($object) && isset($config['create']))
            {
                foreach ($config['create'] as $field)
                {
                    ...
                    $new = \array_key_exists($field, $changeSet) ? $changeSet[$field][1] : null;
                    if ($new === null)
                    { // let manual values
                        $needChanges = true;
            ...
            if (isset($config['update']))
            {
                ...
                {
                    $isInsertAndNull = $uow->isScheduledForInsert($object) && array_key_exists($field, $changeSet) && $changeSet[$field][1] === null;
                    if (! isset($changeSet[$field]) || $isInsertAndNull)
                    { // let manual values
                        $needChanges = true;
                        $this->updateField($object, $ea, $meta, $field);
                    }
    ...
    public function prePersist(EventArgs $args)
    {
        ...
        if ($config = $this->getConfiguration($om, $meta->getName()))
        {
            if (isset($config['update']))
            {
                foreach ($config['update'] as $field)
                {
                    if ($meta->getReflectionProperty($field)
    ...
    protected function updateField($object, $eventAdapter, $meta, $field)
    {
        ...
        if ($meta->hasAssociation($field) && is_object($newValue) && ! $eventAdapter->getObjectManager()
            ->contains($newValue))
        {
            $uow = $eventAdapter->getObjectManager()
                ->getUnitOfWork();
