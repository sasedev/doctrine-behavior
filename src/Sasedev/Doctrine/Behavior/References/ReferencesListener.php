<?php
namespace Sasedev\Doctrine\Behavior\References;
        ...
        if (isset($config['referenceOne']))
        {
            foreach ($config['referenceOne'] as $mapping)
            {
                ...
                if (isset($mapping['identifier']))
                {
                    ...
                    {
                        $property->setValue($object, $ea->getSingleReference($this->getManager($mapping['type']), $mapping['class'], $referencedObjectId));
                    }
                }
            }
        }
            ...
            foreach ($config['referenceMany'] as $mapping)
            {
                ...
                if (isset($mapping['mappedBy']))
                {
                    ...
                    if (isset($refConfig['referenceOne'][$mapping['mappedBy']]))
                    {
                        ...
                        {
                            $results = $manager->getRepository($class)
                                ->findBy(array(
                                $identifier => $id
                            ));
            ...
            foreach ($config['referenceOne'] as $mapping)
            {
                if (isset($mapping['identifier']))
                {
                    ...
                    if (is_object($referencedObject))
                    {
                        ...
                        $meta->setFieldValue($object, $mapping['identifier'], $identifier);
                    }
                }
            }
        }
